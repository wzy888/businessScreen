package com.zhumei.baselib.widget.dialog;
...
public class  PasswordInputView extends androidx.appcompat.widget.AppCompatEditText {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 内容区
        RectF rectIn = new RectF(rect.left + DEFAULT_CONT_MARGIN, rect.top + DEFAULT_CONT_MARGIN,rect.right - DEFAULT_CONT_MARGIN, rect.bottom - DEFAULT_CONT_MARGIN);
        borderPaint.setColor(Color.WHITE);
        canvas.drawRoundRect(rectIn, borderRadius, borderRadius, borderPaint);

        // 分割线
        borderPaint.setColor(borderColor);
        borderPaint.setStrokeWidth(DEFAULT_SPLIT_LINE_WIDTH);
        for (int i = 1; i < passwordLength; i++) {
            float x = width * i / (float)passwordLength;
            canvas.drawLine(x, 0, x, height, borderPaint);
        }

        // 密码
        float cx, cy = (float)height/ 2;
