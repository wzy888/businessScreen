...
//    private static final int NETWORK_STATE_AVAILABLE_WIFI = 2;
    ...
    public void registerNetworkCallback(NetworkChangeListener networkChangeListener) {
        if (hasRegistNetworkCallback()) {
            LogUtils.d("hasRegistNetworkCallback");
            return;
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ConnectivityManager connectivityManager = (ConnectivityManager) BaseHelper.getInstance().getContext().getSystemService(Context.CONNECTIVITY_SERVICE);
